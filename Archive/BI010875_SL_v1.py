...
for q in questions:
    for ans in df_filtered[q].unique():
        ...
        node_colors.append("lightgray")  # all nodes grey
...
for i in range(len(questions)-1):
    ...
    for _, row in counts.iterrows():
        ...
        link_colors.append(answer_colors.get(row[q_from], "lightblue"))  # links colored by answer
...
with col_chart:
    ...
    fig = go.Figure(go.Sankey(
        node=dict(
            label=node_labels,        # default labels
            color=node_colors,             
            pad=10,
            thickness=40,
            line=dict(color='white', width=0)  # remove outline
        ),
        link=dict(
            source=sources,
            target=targets,
    ...
    fig.update_layout(
        ...
        family="Arial",
        color="black",
        size=12  # optional: adjust size as needed
        )   ,
        legend=dict(
            orientation="h",
            yanchor="bottom",
...
# -----------------------
# Questions table underneath
# -----------------------
st.header("Questions")
table_data = [{"Question": f"{question_labels[q]} = {q}"} for q in questions]
